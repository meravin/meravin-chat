...
41 checks covering the parts that are easy to get subtly wrong: a resent UUID
stores once and wakes an AI once, private channels can't leak into each other,
the four AI contexts stay separate, an `@mention` earns exactly one extra round
instead of an infinite loop, a lane stays serial under load, an adapter failure
surfaces a status instead of an endless spinner, the `before` cursor walks
history to the beginning and stops, a `%` in a search query matches literally,
todo extraction proposes without writing, an erase refuses without its
confirmation phrase, and the scheduler writes one whisper a day and prunes old
backups.
...
connect; older ones come from `GET /api/messages?before=<ms>`, driven by scroll
position rather than a sentinel's `onAppear` (which fires once and then stalls,
because the spinner never leaves the screen).
